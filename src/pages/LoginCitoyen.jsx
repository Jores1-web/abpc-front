...
export default function LoginCitoyen(){
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await api.post('/citoyen/login', form)
  ...
  return (
    <div style={authStyle.centerPage}>
      <div style={authStyle.centerCard} className="animate-fadeUp">
        {/* Header */}
        <div style={authStyle.centerHeader}>
          ...
          <h1 style={authStyle.centerTitle}>Connexion</h1>
          <p style={authStyle.centerSubtitle}>Accédez à votre espace citoyen ABPC</p>
        </div>
        ...
        <div style={authStyle.centerFooter}>
          <p>Pas encore de compte ? <Link to="/register" style={authStyle.footerLink}>S'inscrire</Link></p>
          <p style={{ marginTop: 8 }}>
            <Link to="/signaler" style={{ ...authStyle.footerLink, color: 'var(--danger)' }}>
